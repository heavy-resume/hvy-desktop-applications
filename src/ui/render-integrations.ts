...
export function renderIntegrationPageDeleteDialog(state: AppState): string {
  if (!state.integrationPageDeleteDialogOpen) return '';
  ...
  if (!integration || !page) return '';
  return `<div class="modal-backdrop modal-backdrop-stacked" role="presentation"><section class="dialog" role="alertdialog" aria-modal="true" aria-label="Delete page"><h2>Delete ${escapeHtml(page.name)}?</h2><p>This permanently removes the page, its record types, page commands, and item commands. It does not delete data from the website or clear shared browser profiles.</p><div class="dialog-actions"><button type="button" class="hvy-galaxy-button" data-action="cancel-delete-integration-page">Cancel</button><button type="button" class="hvy-galaxy-button danger-button" data-action="confirm-delete-integration-page">Delete Page</button></div></section></div>`;
}

export function renderIntegrationNavigationDialog(state: AppState): string {
  const request = state.integrationNavigationRequest;
  if (!request) return '';
  const currentOrigin = new URL(request.currentUrl).origin;
  const requestedOrigin = new URL(request.requestedUrl).origin;
  const frameAmbiguous = request.navigationKind === 'frame-or-main';
  return `<div class="modal-backdrop modal-backdrop-stacked" role="presentation"><section class="dialog integration-navigation-dialog" role="dialog" aria-modal="true" aria-label="Allow integration navigation"><div class="modal-header"><div><p class="eyebrow">Integration navigation</p><h2>${frameAmbiguous ? 'Allow content from this origin?' : 'Allow this page to leave its configured site?'}</h2></div></div><p><strong>${escapeHtml(currentOrigin)}</strong> requested ${frameAmbiguous ? 'content' : 'navigation'} from a different origin:</p><div class="integration-navigation-destination"><strong>${escapeHtml(requestedOrigin)}</strong><span>${escapeHtml(request.requestedUrl)}</span></div><p class="dialog-note">Allowing saves this origin to the integration page. ${frameAmbiguous ? 'Galaxy will reload the integration page so embedded content remains in its own frame.' : 'Future redirects to it will remain inside Galaxy.'}</p><div class="dialog-actions"><button type="button" class="hvy-galaxy-button" data-action="reject-integration-navigation">Reject</button><button type="button" class="hvy-galaxy-button primary-button" data-action="approve-integration-navigation">${frameAmbiguous ? 'Allow and reload' : 'Allow and continue'}</button></div></section></div>`;
}
...
export function renderIntegrationReadyChecksDialog(state: AppState): string {
  ...
  return `<div class="modal-backdrop modal-backdrop-stacked" role="presentation"><form class="dialog integration-ready-checks-dialog" role="dialog" aria-modal="true" aria-label="Page settings" data-form="integration-ready-checks"><input type="hidden" name="integrationId" value="${escapeAttr(integration.id)}"><input type="hidden" name="pageId" value="${escapeAttr(page.id)}"><div class="modal-header"><div><p class="eyebrow">${escapeHtml(page.name)}</p><h2>Page settings</h2><p class="dialog-note">Configure navigation origins and checks that prevent pulling incorrect data or attempting wrong commands.</p></div><button type="button" class="hvy-galaxy-button icon-button" data-action="cancel-integration-ready-checks" aria-label="Close">×</button></div><section class="integration-allowed-origins"><label><span>Allowed navigation origins</span><textarea class="hvy-galaxy-input integration-allowed-origins-input" name="allowedOrigins" spellcheck="false">${escapeHtml(state.integrationAllowedOriginsDraft)}</textarea></label><p class="field-help">Enter one origin per line. Redirects and new windows remain inside Galaxy only when their origin is listed here.</p></section><section class="integration-ready-url"><label><span>URL check</span><select class="hvy-galaxy-select" name="urlMode" data-action="integration-ready-url-mode" data-page-url="${escapeAttr(page.url)}"><option value="strict-url" ${draft.urlMode === 'strict-url' ? 'selected' : ''}>Strict URL</option><option value="strict-domain" ${draft.urlMode === 'strict-domain' ? 'selected' : ''}>Strict domain</option><option value="domain-regex" ${draft.urlMode === 'domain-regex' ? 'selected' : ''}>Domain regex</option></select></label><label><span>Expected URL or domain</span><input class="hvy-galaxy-input" name="urlValue" required value="${escapeAttr(draft.urlValue)}"></label><p class="field-help" data-ready-url-help>${escapeHtml(modeHelp)}</p></section><section><div class="integration-section-heading"><div><h3>Page landmarks</h3><p>Use page landmarks to ensure you're logged in, etc.</p></div><button type="button" class="hvy-galaxy-button" data-action="add-integration-ready-check" data-integration-id="${escapeAttr(integration.id)}" data-page-id="${escapeAttr(page.id)}">+ Select element</button></div><div class="integration-ready-check-list">${elements}</div></section>${validationSummary}<div class="dialog-actions"><button type="button" class="hvy-galaxy-button danger-button integration-page-delete-button" data-action="request-delete-integration-page" data-integration-id="${escapeAttr(integration.id)}" data-page-id="${escapeAttr(page.id)}">Delete Page</button><button type="button" class="hvy-galaxy-button" data-action="cancel-integration-ready-checks">Cancel</button><button type="button" class="hvy-galaxy-button" data-action="test-integration-ready-checks" data-integration-id="${escapeAttr(integration.id)}" data-page-id="${escapeAttr(page.id)}">Reload page &amp; test</button><button type="submit" class="hvy-galaxy-button primary-button">Save page settings</button></div></form></div>`;
}
