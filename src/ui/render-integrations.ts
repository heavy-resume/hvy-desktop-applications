...
export function renderIntegrationWebMcpInvokeDialog(state: AppState): string {
  ...
  if (state.integrationWebMcpPending) {
    return `<div class="modal-backdrop modal-backdrop-stacked" role="presentation"><section class="dialog integration-webmcp-test-dialog integration-webmcp-pending-dialog" role="dialog" aria-modal="true" aria-label="${configuringRecords ? 'Reading WebMCP records' : 'Running WebMCP tool'}"><h2>${configuringRecords ? 'Reading records…' : 'Running tool…'}</h2><p class="dialog-note">${configuringRecords ? 'Galaxy is opening the page, waiting for its WebMCP tools, and reading a sample result.' : 'Galaxy is waiting for the page to return a result.'}</p></section></div>`;
  }
  return `<div class="modal-backdrop modal-backdrop-stacked" role="presentation"><form class="dialog integration-webmcp-test-dialog" role="dialog" aria-modal="true" aria-label="${configuringRecords ? 'Configure WebMCP record type' : 'Test WebMCP tool'}" data-form="invoke-webmcp-tool"><input type="hidden" name="capabilityId" value="${escapeAttr(capabilityId)}"><h2>${configuringRecords ? 'Configure' : 'Test'} ${escapeHtml(approval.descriptor.title ?? approval.descriptor.name)}</h2><p class="dialog-note">${configuringRecords ? 'Enter the values used to retrieve records. Galaxy will inspect the result before anything is saved.' : 'Enter test values, then run the real tool against the selected integration page and profile.'}</p>${state.integrationWebMcpError ? `<div class="integration-fetch-error" role="alert"><strong>WebMCP failed</strong><span>${escapeHtml(state.integrationWebMcpError)}</span></div>` : ''}${renderWebMcpTestArguments(approval.descriptor.inputSchema)}<div class="dialog-actions"><button type="button" class="hvy-galaxy-button" data-action="cancel-invoke-webmcp-tool">Cancel</button><button type="submit" class="hvy-galaxy-button primary-button">${configuringRecords ? 'Read records' : 'Run Test'}</button></div></form></div>`;
}
...
function renderIntegrationWebMcpRecordBuilder(state: AppState, candidates: WebMcpRecordSetCandidate[]): string {
  ...
  const choices = candidates.map((candidate) => {
    ...
    const fields = candidate.fields.map((field) => `${field.name}${field.presentIn < candidate.records.length ? ' (sometimes)' : ''}`).join(', ');
    return `<label class="integration-webmcp-permission"><input type="radio" name="recordsPath" value="${escapeAttr(candidate.path)}" data-action="select-webmcp-record-path" ${candidate.path === selected.path ? 'checked' : ''}><span class="integration-webmcp-permission-copy"><strong>${escapeHtml(path)}</strong><small>${candidate.records.length} ${candidate.records.length === 1 ? 'record' : 'records'}${fields ? ` · ${escapeHtml(fields)}` : ''}</small></span></label>`;
  }).join('');
  const fields = selected.fields.map((field) => `<div class="integration-webmcp-argument-row"><label class="integration-webmcp-permission"><input type="checkbox" name="recordField" value="${escapeAttr(field.name)}" checked><span class="integration-webmcp-permission-copy"><strong>${escapeHtml(field.name)}</strong><small>${field.valueKinds.join(' or ')}${field.presentIn < selected.records.length ? ` · Present in ${field.presentIn} of ${selected.records.length}` : ''}</small></span></label><input class="hvy-galaxy-input" name="fieldLabel:${escapeAttr(field.name)}" value="${escapeAttr(field.name)}" aria-label="Label for ${escapeAttr(field.name)}"></div>`).join('');
  return `<form class="dialog integration-webmcp-review-dialog" role="dialog" aria-modal="true" aria-label="Save WebMCP record type" data-form="save-webmcp-record-type"><h2>Save as record type</h2><p class="dialog-note">Choose the collection Galaxy should treat as records, then select and name its fields.</p><label><span>Name</span><input class="hvy-galaxy-input" name="recordName" required autocomplete="off" value="${escapeAttr(approval.descriptor.title ?? approval.descriptor.name)}"></label><div class="integration-webmcp-permissions">${choices}</div><div class="integration-webmcp-arguments">${fields || '<span class="integration-empty-value">No fields were present in this result.</span>'}</div><div class="dialog-actions"><button type="button" class="hvy-galaxy-button" data-action="cancel-save-webmcp-record-type">Cancel</button><button type="submit" class="hvy-galaxy-button primary-button" ${fields ? '' : 'disabled'}>Save record type</button></div></form>`;
}
