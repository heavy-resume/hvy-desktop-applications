...
export interface UiHandlers {
  ...
  beginIntegrationCommandSelection(name: string, scope: 'page' | 'record', gesture: 'click' | 'right-click'): void;
...
function bind(root: HTMLElement, handlers: UiHandlers, state: AppState): void {
  ...
  root.addEventListener('submit', (event) => {
    ...
    if (form.dataset.form === 'integration-command-setup') {
      const data = new FormData(form);
      const scope = data.get('commandScope');
      const gesture = data.get('commandGesture');
      if ((scope === 'page' || scope === 'record') && (gesture === 'click' || gesture === 'right-click')) {
        handlers.beginIntegrationCommandSelection(String(data.get('commandName') ?? ''), scope, gesture);
      }
    }
      ...
      <section class="dialog integrations-dialog integrations-manager" role="dialog" aria-modal="true" aria-label="Integrations" data-prevent-dismiss="true">
        ...
        <div class="integrations-manager-body">
          <nav class="integration-list" aria-label="Configured integrations">
            ${state.integrationRegistry.integrations.map((integration) => `<button type="button" class="hvy-galaxy-button integration-list-item ${integration.id === selectedIntegration.id ? 'selected' : ''}" data-action="select-integration" data-integration-id="${escapeAttr(integration.id)}"><strong>${escapeHtml(integration.name)}</strong><span>${integration.pages.length} ${integration.pages.length === 1 ? 'page' : 'pages'} · ${integration.actions.length} ${integration.actions.length === 1 ? 'record' : 'records'}</span></button>`).join('')}
          ...
          <main class="integration-detail">
            ...
            <section><div class="integration-section-heading"><div><h4>Pages</h4><p>Open a page or define reusable records from it.</p></div></div><div class="integration-page-grid">${selectedIntegration.pages.map((page) => `<article class="integration-page-card"><div><strong>${escapeHtml(page.name)}</strong><span>${escapeHtml(new URL(page.url).hostname)}</span></div><div class="integration-page-actions"><button class="hvy-galaxy-button" type="button" data-action="open-integration-page" data-integration-id="${escapeAttr(selectedIntegration.id)}" data-page-id="${escapeAttr(page.id)}">Open</button><button type="button" class="hvy-galaxy-button primary-button" data-action="add-action-for-integration-page" data-integration-id="${escapeAttr(selectedIntegration.id)}" data-page-id="${escapeAttr(page.id)}">Define records</button></div></article>`).join('')}</div></section>
            <section><div class="integration-section-heading"><div><h4>Records</h4><p>Reusable structured records and the commands that operate on them.</p></div></div>${selectedIntegration.actions.length ? `<div class="integration-action-list">${selectedIntegration.actions.map((action) => `<article class="integration-record-definition"><div class="integration-record-summary"><strong>${escapeHtml(action.name)}${action.status === 'draft' ? ' <small>Draft</small>' : ''}</strong><span>${escapeHtml(action.description || action.pattern?.fields.map((field) => field.label).join(', ') || '')}</span>${action.commands?.length ? `<div class="integration-command-list">${action.commands.map((command) => `<span><strong>${escapeHtml(command.name)}</strong><small>${command.scope === 'record' ? 'Item' : 'Page'} · ${command.steps[0]?.gesture === 'right-click' ? 'Right click' : 'Click'}</small></span>`).join('')}</div>` : '<small>No commands yet</small>'}</div><div class="integration-record-actions"><button type="button" class="hvy-galaxy-button" data-action="add-command-for-integration-action" data-integration-id="${escapeAttr(selectedIntegration.id)}" data-action-id="${escapeAttr(action.id)}">Add command</button><button type="button" class="hvy-galaxy-button primary-button" data-action="run-integration-action" data-integration-id="${escapeAttr(selectedIntegration.id)}" data-action-id="${escapeAttr(action.id)}" ${action.pattern ? '' : 'disabled'}>Fetch items</button></div></article>`).join('')}</div>` : '<div class="integration-empty-state"><strong>No records yet</strong><span>Choose Define records beside a page to create one.</span></div>'}</section>
          </main>
        </div>
      </section>
    </div>`;
...
function renderExtractionRecords(records: unknown[], commandContext?: { integrationId: string; actionId: string; commands: NonNullable<import('./integrationRegistry').IntegrationActionDefinition['commands']> }): string {
